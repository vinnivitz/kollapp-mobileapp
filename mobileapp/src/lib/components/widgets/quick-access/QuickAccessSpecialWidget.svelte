<script lang="ts">
	...
	} from '$lib/components/widgets/quick-access/widgets';
...
>
	{#if item.widgetType === 'special'}
		{#if item.specialWidgetId === 'organization-card'}
			<OrganizationWidget {organization} {editMode} />
		{:else if item.specialWidgetId === 'upcoming-activity-card'}
			<UpcomingActivityWidget {activity} {editMode} />
		{:else if item.specialWidgetId === 'budget-chart-card'}
			<BudgetChartWidget {editMode} {postings} tourId={TourStepId.HOME.BUDGET_CHART} />
		{/if}
	{/if}
